...
module.exports = class PgPersistence {
  ...
  async registerUser(username, password) {
    ...
    const CREATE_USER = "INSERT INTO users" + 
                        "(username, password)" + 
                        "VALUES ($1, $2)";
    let result = await dbQuery(CREATE_USER, username, hashedPassword);
    return result.rowCount > 0;
  }
  ...
  async deleteTodo(todoListId, todoId) {
    let DELETE_TASK = 'DELETE FROM todos WHERE todolist_id = $1 AND id = $2 AND username = $3';
    
    let result = await dbQuery(DELETE_TASK, todoListId, todoId, this.username);
    return result.rowCount > 0;
  }
  ...
  async deleteTodoList(todoListId) {
    let DELETE_LIST = 'DELETE FROM todoLists WHERE id = $1 AND username = $2';
